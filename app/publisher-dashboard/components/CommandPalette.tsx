...
import { Search, Gamepad2, Monitor, MapPin, BarChart2, Radio, GitBranch, X } from "lucide-react";
...
const SECTIONS = [
  ...
  { id: "section-sentiment",   label: "Sentiment Radar",             icon: <Radio     className="w-3.5 h-3.5" /> },
  { id: "section-automation",  label: "Automation Pipeline",         icon: <GitBranch className="w-3.5 h-3.5" /> },
  { id: "section-revenue",     label: "Regional Revenue Map",        icon: <MapPin    className="w-3.5 h-3.5" /> },
];

export default function CommandPalette({
  onNavigate, onTogglePresentation, presentationMode, onHighlightGame,
}: CommandPaletteProps) {
              ...
              >
                <Command className="w-full">
                  ...
                  <div className="flex items-center gap-3 px-4 py-3.5 border-b border-white/08">
                    <Search className="w-4 h-4 text-brand-4 flex-shrink-0" />
                    <Command.Input
                      placeholder="Search games, jump to section, toggle mode…"
                  ...
                  <Command.List className="max-h-[380px] overflow-y-auto py-2">
                    ...
                    {/* Games */}
                    <Command.Group heading="Games" className="[&>[cmdk-group-heading]]:px-4 [&>[cmdk-group-heading]]:py-1.5 [&>[cmdk-group-heading]]:text-[10px] [&>[cmdk-group-heading]]:font-bold [&>[cmdk-group-heading]]:uppercase [&>[cmdk-group-heading]]:tracking-widest [&>[cmdk-group-heading]]:text-brand-4">
                      {GAMES.map((game) => (
                        <Command.Item
                          key={game.id}
                          value={game.name}
                          onSelect={() => handleGame(game.id)}
                          className="flex items-center gap-3 px-4 py-2.5 cursor-pointer text-[13px] text-brand-3 hover:text-brand-1 data-[selected=true]:bg-white/05 data-[selected=true]:text-brand-1 rounded-lg mx-2 transition-colors"
                        >
                          <span
                    ...
                    {/* Sections */}
                    <Command.Group heading="Jump To" className="[&>[cmdk-group-heading]]:px-4 [&>[cmdk-group-heading]]:py-1.5 [&>[cmdk-group-heading]]:text-[10px] [&>[cmdk-group-heading]]:font-bold [&>[cmdk-group-heading]]:uppercase [&>[cmdk-group-heading]]:tracking-widest [&>[cmdk-group-heading]]:text-brand-4">
                      {SECTIONS.map((s) => (
                        <Command.Item
                          key={s.id}
                          value={s.label}
                    ...
                    {/* Actions */}
                    <Command.Group heading="Actions" className="[&>[cmdk-group-heading]]:px-4 [&>[cmdk-group-heading]]:py-1.5 [&>[cmdk-group-heading]]:text-[10px] [&>[cmdk-group-heading]]:font-bold [&>[cmdk-group-heading]]:uppercase [&>[cmdk-group-heading]]:tracking-widest [&>[cmdk-group-heading]]:text-brand-4">
                      <Command.Item
